...
// The zPLSE block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zPLSE is a hash of the serial
...
{
    //Create an output returning the zPLSE that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZPLSEOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zPLSE output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zPLSE", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZPLSEOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zPLSE output", __func__);
...
{
    CzPLSETracker* zplseTracker = pwallet->zplseTracker.get();
    CMintMeta meta;
    if (!zplseTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zplseTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a PLSE stake is the outpoint
