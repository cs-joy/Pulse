...
CzPLSETracker::CzPLSETracker(std::string strWalletFile)
{
...
CzPLSETracker::~CzPLSETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzPLSETracker::Init()
{
...
bool CzPLSETracker::Archive(CMintMeta& meta)
{
...
bool CzPLSETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzPLSETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzPLSETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzPLSETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzPLSETracker::GetSerialHashes()
{
...
CAmount CzPLSETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzPLSETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzPLSETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzPLSETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzPLSETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzPLSETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzPLSETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzPLSETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzPLSETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzPLSETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzPLSETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPLSETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPLSETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzPLSETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzPLSETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzPLSETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzPLSETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzplse from DB\n", __func__, listDeterministicDB.size());
    }
