...
CzPLSEWallet::CzPLSEWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zplse seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPLSESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPLSESeed_deprecated()) {
                    LogPrintf("%s: Updated zPLSE seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zplse seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zplse wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPLSEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPLSECount(nCountLastUsed);
    else if (!walletdb.ReadZPLSECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPLSEWallet::Lock()
{
    seedMaster = 0;
}

void CzPLSEWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPLSEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPLSE(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPLSEWallet::LoadMintPoolFromDB()
{
...
void CzPLSEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPLSEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPLSEWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zplseTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPLSEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPLSE(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zplseTracker which also adds to database
    pwalletMain->zplseTracker->Add(dMint, true);
...
void CzPLSEWallet::SeedToZPLSE(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPLSEWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPLSEWallet::GenerateDeterministicZPLSE(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPLSEWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPLSEWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
