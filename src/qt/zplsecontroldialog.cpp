...
#include "ui_zplsecontroldialog.h"
...
std::set<std::string> ZSpdControlDialog::setSelectedMints;
std::set<CMintMeta> ZSpdControlDialog::setMints;

bool CZSpdControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZSpdControlDialog::COLUMN_DENOMINATION || column == ZSpdControlDialog::COLUMN_VERSION || column == ZSpdControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZSpdControlDialog::ZSpdControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZSpdControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSpdControlDialog::~ZSpdControlDialog()
{
    delete ui;
}

void ZSpdControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSpdControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZSpdControlWidgetItem* itemDenom(new CZSpdControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZSpdControlWidgetItem *itemMint = new CZSpdControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZSpdControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSpdControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZSpd_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSpdControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZSpdControlDialog::GetSelectedMints()
{
...
void ZSpdControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
