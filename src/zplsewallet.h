...
#ifndef PULSE_ZPLSEWALLET_H
#define PULSE_ZPLSEWALLET_H
...
public:
    CzPLSEWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZPLSE(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZPLSE(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //PULSE_ZPLSEWALLET_H
