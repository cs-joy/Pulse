...
#if defined(NDEBUG)
#error "Pulse cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uplse) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
   ...
   if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
       return error("%s : zPLSE spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zPLSE is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zPLSE spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zPLSE without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zPLSE spend with serial %s from tx %s is not in valid range\n", __func__,
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE) && tx.ContainsZerocoins())
        return state.DoS(10, error("AcceptToMemoryPool : Zerocoin transactions are not supported by Pulse"), REJECT_INVALID, "bad-tx");
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zPLSE spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zplse");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zPLSE mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight < 5000 && nHeight >= 1000) {
		nSubsidy = 25 * COIN;
	}
	...
	else if (nHeight < 30000 && nHeight >= 25000) {
		nSubsidy = 12 * COIN;
	}
	...
	else if (nHeight < 350000 && nHeight >= 320000) {
		nSubsidy = 17 * COIN;
	}
	...
	else if (nHeight < 450000 && nHeight >= 425000) {
		nSubsidy = 15 * COIN;
	}
	else if (nHeight < 475000 && nHeight >= 450000) {
		nSubsidy = 14 * COIN;
	}
	else if (nHeight < 500000 && nHeight >= 475000) {
		nSubsidy = 14.5 * COIN;
	}
	...
	else if (nHeight < 725000 && nHeight >= 700000) {
		nSubsidy = 12.5 * COIN;
	}
	else if (nHeight < 750000 && nHeight >= 725000) {
		nSubsidy = 12 * COIN;
	}
	else if (nHeight < 775000 && nHeight >= 750000) {
		nSubsidy = 10.5 * COIN;
	}
	else if (nHeight < 800000 && nHeight >= 775000) {
		nSubsidy = 10 * COIN;
	}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZPLSEStake)
{
	int64_t ret = 0;
	...
	if (nHeight < 100) {
	...
	else if (nHeight < 5000 && nHeight >= 100) {
		ret = blockValue * 6 / 10;  //60%;
	}
	else if (nHeight < 10000 && nHeight >= 5000) {
		ret = blockValue * 8 / 10;  //80%;
	}
	else if (nHeight < 15000 && nHeight >= 10000) {
		...
		ret = blockValue * 181 / 200;  //90.5%;
	}
	else if (nHeight < 425000 && nHeight >= 400000) {
		ret = blockValue * 91 / 100;  //91%;
	}
	else if (nHeight < 450000 && nHeight >= 425000) {
		ret = blockValue * 183 / 200;  //91.5%;
	}
	else {
		ret = blockValue * 23 / 25;  //92%;
	}
	return ret;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Pulse
...
{
    RenameThread("pulse-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZPLSEMinted()
{
...
void RecalculateZPLSESpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zPLSE supply
...
bool RecalculatePLSESupply(int nHeightStart)
{
...
{
    // Pulse: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZPLSESupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (block.nTime > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE) && !IsInitialBlockDownload() && tx.ContainsZerocoins()) {
            return state.DoS(100, error("ConnectBlock() : zerocoin transactions are not supported by Pulse"));
        }
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZPLSEMinted();
        RecalculateZPLSESpent();
        RecalculatePLSESupply(Params().Zerocoin_StartHeight());
    }

    //Track zPLSE money supply in the block index
    if (!UpdateZPLSESupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zPLSE supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zSpdSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert PLSE to zPLSE
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zPLSE spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zPLSE serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zPLSE stake block is less than allowed start height", __func__);

    if (CZSpdStake* zPLSE = dynamic_cast<CZSpdStake*>(stake)) {
        CBlockIndex* pindexFrom = zPLSE->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zPLSE stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
             return error("%s: zPLSE stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zPLSE->GetValue()));
        if (nChecksum200 != zPLSE->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zPLSE->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZPLSE() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zPLSE fails context checks", __func__));
    ...
    if (isPoS) {
       ...
       // Inputs
       std::vector<CTxIn> plseInputs;
       std::vector<CTxIn> zPLSEInputs;

       for (const CTxIn& stakeIn : stakeTxIn.vin) {
           if(stakeIn.scriptSig.IsZerocoinSpend()){
               zPLSEInputs.push_back(stakeIn);
           }else{
               plseInputs.push_back(stakeIn);
           }
       }
       const bool hasPLSEInputs = !plseInputs.empty();
       const bool hasZPLSEInputs = !zPLSEInputs.empty();
       ...
       for (const CTransaction& tx : block.vtx) {
           for (const CTxIn& in: tx.vin) {
               ...
               if(hasPLSEInputs)
                   ...
                   for (const CTxIn& plseIn : plseInputs){
                       if(plseIn.prevout == in.prevout){
       ...
       if (isBlockFromFork) {
           ...
           do {
               ...
               for (const CTransaction& t : bl.vtx) {
                   for (const CTxIn& in: t.vin) {
                       ...
                       for (const CTxIn& stakeIn : plseInputs) {
           ...
           // Now that this loop if completed. Check if we have zPLSE inputs.
           if(hasZPLSEInputs){
               for (const CTxIn& zPivInput : zPLSEInputs) {
                   ...
                   if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                       return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                  stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zplse");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zPLSE mints and %d zPLSE spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Pulse: We use certain sporks during IBD, so check to see if they are
