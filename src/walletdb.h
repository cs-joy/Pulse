...
public:
    ...
    bool WriteZPLSESeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZPLSESeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZPLSESeed_deprecated(uint256& seed);
    bool EraseZPLSESeed();
    bool EraseZPLSESeed_deprecated();

    bool WriteZPLSECount(const uint32_t& nCount);
    bool ReadZPLSECount(uint32_t& nCount);
