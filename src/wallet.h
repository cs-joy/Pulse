...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT5000IFMN = 3,
    ONLY_NONDENOMINATED_NOT5000IFMN = 4, // ONLY_NONDENOMINATED and not 5000 PLSE at the same time
    ONLY_5000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zPLSE send
enum ZerocoinSpendStatus {
    ZPLSE_SPEND_OKAY = 0,                            // No error
    ZPLSE_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZPLSE_WALLET_LOCKED = 2,                         // Wallet was locked
    ZPLSE_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZPLSE_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZPLSE_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZPLSE_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZPLSE_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZPLSE_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZPLSE_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZPLSE_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZPLSE_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZPLSE_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZPLSE_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZPLSE_SPENT_USED_ZPLSE = 14,                      // Coin has already been spend
    ZPLSE_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZPLSE_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZSpdBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZPLSEOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzplseAuto) const;
    ...
    */
    ...
    CzPLSEWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzPLSETracker> zplseTracker;
    ...
    void setZWallet(CzPLSEWallet* zwallet)
    {
        zwalletMain = zwallet;
        zplseTracker = std::unique_ptr<CzPLSETracker>(new CzPLSETracker(strWalletFile));
    }

    CzPLSEWallet* getZWallet() { return zwalletMain; }
    ...
    void setZSpdAutoBackups(bool fEnabled)
    {
    ...
    /** zPLSE reset */
    boost::signals2::signal<void()> NotifyzPLSEReset;
